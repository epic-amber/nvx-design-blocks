...
const SECTION_IDS = ["hero", "text-img", "features"] as const;
...
/**
 * Sticky top navigation with section links and Design System link.
...
export function NavBar() {
  ...
  return (
    ...
    >
      <nav className="mx-auto flex h-12 min-h-[44px] max-w-7xl items-center justify-between gap-4 px-4 text-sm sm:h-14 sm:gap-6 sm:px-6 lg:gap-8 lg:px-8">
        <Link
          href="/"
          className="flex flex-shrink-0 items-center gap-2 text-text-inverse no-underline transition-opacity hover:opacity-90 focus:outline-none"
          aria-label="Design blocks — home"
        >
          <img src="/logo.svg" alt="" width={24} height={24} className="h-6 w-6 shrink-0" />
          <span className="font-display font-semibold">Design blocks</span>
        </Link>
        ...
        {/* Mobile: burger and overlay when open */}
